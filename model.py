...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    # Add corpus tokens in first-seen order
...
def build_causal_mask(seq_len):
    ...
    return torch.tril(torch.ones(seq_len, seq_len)).bool()[None, None, :, :]
...
def merge_heads_back_to_model_dim(multi_head_tensor):
    ...
    return multi_head_tensor.transpose(1, 2).reshape(B, L, H * D)
...
def assemble_multi_head_attention_forward(query, key, value, w_q, w_k, w_v, w_o, num_heads, mask=None):
    ...
    context, weigh = multi_head_scaled_dot_product_attention(q, k, v, mask)

    out = merge_heads_and_project_output(context, w_o, None)
